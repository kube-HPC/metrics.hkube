...
class Tracer {
    ...
    startSpan(options) {
        ...
        const spanOptions = {};
        if (options.parentRelationship && this._spanStack.length > 0) {
            switch (options.parentRelationship) {
                case parentRelationships.childOf:
                    spanOptions.childOf = this._tracer.extract(opentracing.FORMAT_TEXT_MAP, this.topSpan.context());
                    break;
                case parentRelationships.follows:
                    spanOptions.references = [opentracing.followsFrom(
                            this._tracer.extract(opentracing.FORMAT_TEXT_MAP, this.topSpan.context()))]
                    break;
            }
